  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      if (profile) {
        ...
        setSubaccountCode(profile.subaccount_code ?? null);
        setIsOfficial(profile.is_official ?? false);
      }
      ...
      if (userListings) setLocalListings(userListings);
      ...
      if (userOrders) setLocalOrders(userOrders);
    ...
    // ✅ Set up realtime subscriptions for account data
    ...
    const listingsChannel = supabase
      .channel('account-listings')
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'listings', filter: `seller_id=eq.${user.id}` },
        (payload) => {
          setLocalListings(prev => [payload.new, ...prev]);
          toast.success("New listing created!", { duration: 2000 });
        }
      )
      .on('postgres_changes',
      ...
      .on('postgres_changes',
        { event: 'DELETE', schema: 'public', table: 'listings', filter: `seller_id=eq.${user.id}` },
        (payload) => {
          setLocalListings(prev => prev.filter(l => l.id !== payload.old.id));
          toast.success("Listing removed", { duration: 2000 });
        }
      )
      .subscribe();
        ...
        (payload) => {
          if (payload.new.buyer_id === user.id || payload.new.seller_id === user.id) {
            ...
            if (payload.new.status === 'shipped') {
              toast.info("Your order has been shipped!", { duration: 3000 });
            } else if (payload.new.status === 'delivered') {
              toast.success("Order delivered! Enjoy your sneakers!", { duration: 3000 });
            }
          }
        }
      )
        ...
        {pageLoading ? (
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              {/* Seller Orders - using localOrders for real-time updates */}
              {!isGuest && canSell && activeTab === "orders" && (
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <p className="text-sm text-muted-foreground">{localOrders.length} {localOrders.length === 1 ? "order" : "orders"} received</p>
                  </div>
                      ...
                      {localOrders.map((order, i) => (
                        <motion.div key={order.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                          ...
                          <div className="border-t border-border pt-4 space-y-3">
                            ...
                            {/* Tracking number input — verified/official sellers only */}
                              ...
                              <div>
                                ...
                                {order.trackingNumber ? (
                                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20">
                                    ...
                                    <span className="text-xs font-semibold text-blue-600 flex-1">{order.trackingNumber}</span>
                                    <button onClick={() => setTrackingInputs(p => ({ ...p, [order.id]: order.trackingNumber ?? "" }))}
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        ...
                                        if (!num) return;
                                        setSavingTracking(p => ({ ...p, [order.id]: true }));
                                        await addTracking(order.id, num);
                                        setSavingTracking(p => ({ ...p, [order.id]: false }));
                                      }}
              ...
              {/* Buyer Orders - using localOrders for real-time updates */}
              {!isGuest && !canSell && activeTab === "orders" && (
                <div>
                  {localOrders.length === 0 ? (
                    <div className="text-center py-20">
                    ...
                    <div className="space-y-4">
                      <p className="text-sm text-muted-foreground mb-2">{localOrders.length} {localOrders.length === 1 ? "order" : "orders"} placed</p>
                      {localOrders.map((order, i) => (
                        <motion.div key={order.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: i * 0.06 }} className="rounded-2xl border border-border p-5">
                          <div className="flex items-start justify-between gap-3 mb-4">
              ...
              {/* Seller Listings - using localListings for real-time updates */}
              {!isGuest && canSell && activeTab === "listings" && (
                <div>
                  ...
                  <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
                    <div>
                      <p className="text-sm text-muted-foreground">
                        <span className="text-foreground font-semibold">{localListings.filter(l => l.status === "active").length}</span> active{" · "}
                        <span className="text-muted-foreground">{localListings.filter(l => l.status === "sold").length} sold</span>
                      </p>
                  ...
                  {localListings.length === 0 && !showFirstListingBanner && <FirstListingEmptyState onStart={startFirstListing} />}
                  {localListings.length === 0 && showFirstListingBanner && (
